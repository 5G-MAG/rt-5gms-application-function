...
void msaf_m1_state_functional(ogs_fsm_t * s, msaf_event_t * e) {
    ...
    const nf_server_app_metadata_t app_metadata = {
            MSAF_NAME,
            MSAF_VERSION,
            nf_name
    };
    ...
    switch (e -> h.id) {
        ...
        case OGS_EVENT_SBI_SERVER:
            ...
            if (strcmp(message -> h.api.version, "v2") != 0) {
                ...
                ogs_error("%s", error);
                ogs_assert(true == nf_server_send_error(stream, OGS_SBI_HTTP_STATUS_BAD_REQUEST, 1, NULL,
                                                        "Not supported version", error, NULL, NULL, app_meta));
                ogs_free(error);
                break;
            }
            if (!message -> h.resource.component[0]) {
                ...
                ogs_error(error);
                ogs_assert(true == nf_server_send_error(stream, 404, 1, NULL, "No resource given", error, NULL, NULL,
                                                        app_meta));
                break;
            }
            ...
            if (message -> h.resource.component[1] && message -> h.resource.component[2] &&
                message -> h.resource.component[3] && !message -> h.resource.component[4]) {
                ...
                if (!strcmp(message -> h.resource.component[2], "content-hosting-configuration") &&
                    !strcmp(message -> h.resource.component[3], "purge")) {
                    ogs_hash_index_t * hi;
                    for (hi = ogs_hash_first(request -> http.headers); hi; hi = ogs_hash_next(hi)) {
                        if (!ogs_strcasecmp(ogs_hash_this_key(hi), OGS_SBI_CONTENT_TYPE)) {
                            if (ogs_strcasecmp(ogs_hash_this_val(hi), "application/x-www-form-urlencoded")) {
                                ...
                                err = ogs_msprintf(
                                        "Unsupported Media Type: received type: %s, should have been application/x-www-form-urlencoded",
                                        type);
                                ogs_error("%s", err);

                                ogs_assert(true ==
                                           nf_server_send_error(stream, 415, 3, message, "Unsupported Media Type.", err,
                    ...
                    if (msaf_provisioning_session) {
                        ...
                        ogs_list_for_each( & msaf_provisioning_session -> application_server_states, as_state_ref) {
                            ...
                            if (as_state -> application_server && as_state -> application_server -> canonicalHostname) {
                                ogs_list_for_each( & as_state -> assigned_provisioning_sessions,
                                                   assigned_provisioning_sessions_resource) {
                                    if (assigned_provisioning_sessions_resource -> assigned_provisioning_session ==
                                        msaf_provisioning_session) {

                                        purge_cache = ogs_calloc(1, sizeof(purge_resource_id_node_t));
                                        ogs_assert(purge_cache);
                                        purge_cache -> provisioning_session_id = msaf_strdup(
                                                assigned_provisioning_sessions_resource -> assigned_provisioning_session -> provisioningSessionId);
                                        ...
                                        char * err = NULL;
                                        err = ogs_msprintf(
                                                "Provisioning Session [%s] is not assigned to an Application Server",
                                                message -> h.resource.component[1]);
                                        ogs_error("%s", err);
                                        ogs_assert(true == nf_server_send_error(stream, 500, 3, message,
                                                                                "Provisioning session is not assigned to an Application Server.",
                                                                                err, NULL,
                                                                                m1_contenthostingprovisioning_api,
                                                                                app_meta));
                                        ogs_free(err);
                                    }
                                }
                                ...
                                char * err = NULL;
                                err = ogs_msprintf(
                                        "Provisioning Session [%s] : Unable to get information about Application Server",
                                        message -> h.resource.component[1]);
                                ogs_error("%s", err);
                                ogs_assert(true == nf_server_send_error(stream, 500, 3, message,
                                                                        "Unable to get information about Application Server",
                                                                        err, NULL, m1_contenthostingprovisioning_api,
                                                                        app_meta));
                                ogs_free(err);
                            }
                        ...
                        char * err = NULL;
                        err = ogs_msprintf("Provisioning session [%s] does not exist.",
                                           message -> h.resource.component[1]);
                        ogs_error("%s", err);
                        ogs_assert(true ==
                                   nf_server_send_error(stream, 404, 3, message, "Provisioning session does not exist.",
                ...
                if (!strcmp(message -> h.resource.component[2], "metrics-reporting-configurations")) {
                    ...
                    if (msaf_provisioning_session) {
                        ...
                        if (!metrics_reporting_config) {
                            char * err = NULL;
                            err = ogs_msprintf(
                                    "Unable to parse Metrics Reporting Configuration as JSON for the Provisioning Session [%s].",
                                    message -> h.resource.component[1]);
                            ogs_error("%s", err);
                            ...
                            msaf_metrics_reporting_configuration_t * new_mrc = msaf_metrics_reporting_configuration_create(
                                    msaf_provisioning_session,
                            ...
                            if (new_mrc) {
                                if (msaf_application_server_state_set_on_post(msaf_provisioning_session)) {
                                    ...
                                    if (mrc_json) {
                                        ...
                                        char * err = NULL;
                                        err = ogs_msprintf(
                                                "Unable to retrieve the Metrics Reporting Configuration for the Provisioning Session [%s].",
                                                message -> h.resource.component[1]);
                                        ogs_error("%s", err);
                                        ogs_assert(true == nf_server_send_error(stream,
                                                                                404,
                                    ...
                                    char * err = NULL;
                                    err = ogs_msprintf(
                                            "Verification error on Metrics Reporting Configuration for the Provisioning Session [%s].",
                                            message -> h.resource.component[1]);
                                    ogs_error("%s", err);
                                    ogs_assert(true == nf_server_send_error(stream, 400, 2, message,
                                                                            "Bad Metrics Reporting Configuration.", err,
                                ...
                                char * err = NULL;
                                err = ogs_msprintf(
                                        "Creation of the Metrics Reporting Configuration failed for the Provisioning Session [%s]",
                                        message -> h.resource.component[1]);
                                ogs_error("%s", err);
                                ogs_assert(true == nf_server_send_error(stream, 500, 2, message,
                                                                        "Creation of the Metrics Reporting Configuration failed.",
                                                                        err, NULL, m1_metricsreportingprovisioning_api,
                                                                        app_meta));
                                ogs_free(err);
                            }
                        }
                        ...
                        ogs_error("%s", err);
                        ogs_assert(true ==
                                   nf_server_send_error(stream, 404, 2, message, "Provisioning session does not exist.",
                                                        err, NULL, m1_metricsreportingprovisioning_api, app_meta));
                        ogs_free(err);
                    }
                }
                ...
                if (!strcmp(message -> h.resource.component[2], "content-hosting-configuration")) {
                    ...
                    if (msaf_provisioning_session) {
                        ...
                        if (!content_hosting_config) {
                            char * err = NULL;
                            err = ogs_msprintf(
                                    "Unable to parse Content Hosting Configuration as JSON for the Provisioning Session [%s].",
                                    message -> h.resource.component[1]);
                            ogs_error("%s", err);
                            ogs_assert(true == nf_server_send_error(stream, 400, 2, message,
                                                                    "Bad Content Hosting Configuration.", err, NULL,
                                                                    m1_contenthostingprovisioning_api, app_meta));
                            ogs_free(err);
                        } else {
                            ...
                            if (rv) {
                                ...
                                if (msaf_application_server_state_set_on_post(msaf_provisioning_session)) {
                                    ...
                                    if (chc != NULL) {
                                        ...
                                        char * err = NULL;
                                        err = ogs_msprintf(
                                                "Unable to retrieve the Content Hosting Configuration for the Provisioning Session [%s].",
                                                message -> h.resource.component[1]);
                                        ogs_error("%s", err);
                                    ...
                                    char * err = NULL;
                                    err = ogs_msprintf(
                                            "Verification error on Content Hosting Configuration for the Provisioning Session [%s].",
                                            message -> h.resource.component[1]);
                                    ogs_error("%s", err);
                                    ogs_assert(true == nf_server_send_error(stream, 400, 2, message,
                                                                            "Bad Content Hosting Configuration.", err,
                                                                            NULL, m1_contenthostingprovisioning_api,
                                                                            app_meta));
                                    ogs_free(err);
                                }
                                ...
                                char * err = NULL;
                                err = ogs_msprintf(
                                        "Creation of the Content Hosting Configuration failed for the Provisioning Session [%s]",
                                        message -> h.resource.component[1]);
                                ogs_error("%s", err);
                                ogs_assert(true == nf_server_send_error(stream, 500, 2, message,
                ...
                if (!strcmp(message -> h.resource.component[2], "certificates")) {
                    ...
                    for (hi = ogs_hash_first(request -> http.params); hi; hi = ogs_hash_next(hi)) {
                    ...
                    if (msaf_provisioning_session) {
                        ...
                        if (csr) {
                            ...
                            ogs_hash_set(msaf_provisioning_session -> certificate_map, msaf_strdup(csr_cert -> id),
                                         OGS_HASH_KEY_STRING, msaf_strdup(csr_cert -> id));
                            ogs_sbi_response_t * response;
                            ...
                            response = nf_server_new_response(location, "application/x-pem-file",
                                                              csr_cert -> last_modified,
                                                              csr_cert -> server_certificate_hash,
                                                              m1_server_certificates_response_max_age, NULL,
                                                              m1_servercertificatesprovisioning_api, app_meta);

                            nf_server_populate_response(response, strlen(csr_cert -> certificate),
                                                        msaf_strdup(csr_cert -> certificate), 200);
                        ...
                        if (cert != NULL) {
                            ...
                            ogs_hash_set(msaf_provisioning_session -> certificate_map, msaf_strdup(cert),
                                         OGS_HASH_KEY_STRING, cert);

                            location = ogs_msprintf("%s/%s", request -> h.uri, cert);
                            response = nf_server_new_response(location,
                                                              NULL,
                                                              0,
                                                              NULL,
                                                              0,
                            ...
                            ogs_hash_set(msaf_provisioning_session -> certificate_map, msaf_strdup(new_cert -> id),
                                         OGS_HASH_KEY_STRING, msaf_strdup(new_cert -> id));
                            ...
                            response = nf_server_new_response(location, NULL, new_cert -> last_modified,
                                                              new_cert -> server_certificate_hash,
                                                              m1_server_certificates_response_max_age, NULL,
                                                              m1_servercertificatesprovisioning_api, app_meta);
                            nf_server_populate_response(response, 0, NULL, 200);
                            ogs_assert(response);
                        ...
                        char * err = NULL;
                        err = ogs_msprintf("Provisioning session [%s] does not exists.",
                                           message -> h.resource.component[1]);
                        ogs_error("%s", err);
                        ogs_assert(true == nf_server_send_error(stream, 404, 2, message,
                ...
                if (msaf_provisioning_session) {
                    char * err = NULL;
                    err = ogs_msprintf("Method [%s] not allowed for [%s].", message -> h.method,
                                       message -> h.resource.component[1]);
                    ogs_error("%s", err);
                    ogs_assert(true == nf_server_send_error(stream, 405, 1, message, "Method not allowed.", err, NULL,
                    ...
                    ogs_error("%s", err);
                    ogs_assert(true ==
                               nf_server_send_error(stream, 404, 1, message, "Provisioning session does not exist.",
                                                    err, NULL, m1_provisioningsession_api, app_meta));
                    ogs_free(err);
                }
                ...
                if (!prov_sess) {
                    ...
                    ogs_error(err);
                    ogs_assert(true == nf_server_send_error(stream, 400, 1, message,
                                                            "Creation of the Provisioning session failed.", err, NULL,
                                                            m1_provisioningsession_api, app_meta));
                    break;
                }
                ...
                if (entry) {
                    if (!cJSON_IsString(entry)) {
                        ...
                        ogs_error(err);
                        ogs_assert(true == nf_server_send_error(stream, 400, 1, message,
                                                                "Creation of the Provisioning session failed.", err,
                                                                NULL, m1_provisioningsession_api, app_meta));
                        break;
                    }
                    asp_id = entry -> valuestring;
                }

                msaf_provisioning_session = msaf_provisioning_session_create(provisioning_session_type, asp_id,
                                                                             external_app_id);
                provisioning_session = msaf_provisioning_session_get_json(
                        msaf_provisioning_session -> provisioningSessionId);
                if (provisioning_session != NULL) {
                    ...
                    if (request -> h.uri[strlen(request -> h.uri) - 1] != '/') {
                        location = ogs_msprintf("%s/%s", request -> h.uri,
                                                msaf_provisioning_session -> provisioningSessionId);
                    } else {
                        location = ogs_msprintf("%s%s", request -> h.uri,
                                                msaf_provisioning_session -> provisioningSessionId);
                    ...
                    response = nf_server_new_response(location,
                                                      "application/json",
                                                      msaf_provisioning_session -> provisioningSessionReceived,
                                                      msaf_provisioning_session -> provisioningSessionHash,
                                                      msaf_self() -> config.server_response_cache_control -> m1_provisioning_session_response_max_age,
                                                      NULL,
                                                      m1_provisioningsession_api,
                                                      app_meta);
                    ...
                    ogs_error(err);
                    ogs_assert(true == nf_server_send_error(stream, 404, 1, message,
                                                            "Creation of the Provisioning session failed.", err, NULL,
                                                            m1_provisioningsession_api, app_meta));
                }
            ...
            if (message -> h.resource.component[1] && message -> h.resource.component[2] &&
                ...
                if (!strcmp(message -> h.resource.component[2], "metrics-reporting-configurations")) {
                    ...
                    if (msaf_provisioning_session) {
                        ...
                        metricsReportingConfigurationId = ogs_hash_get(msaf_provisioning_session -> metricsReportingMap,
                                                                       message -> h.resource.component[3],
                                                                       OGS_HASH_KEY_STRING);
                    ...
                    ogs_assert(true == nf_server_send_error(stream,
                                                            404,
                                                            3, message,
                                                            "Unknown error or invalid request.",
                                                            NULL,
                                                            NULL,
                                                            m1_metricsreportingprovisioning_api,
                                                            app_meta));
                ...
                if (!strcmp(message -> h.resource.component[2], "certificates")) {
                    ...
                    if (msaf_provisioning_session) {
                        ...
                        if (!cert -> return_code) {
                            ...
                            response = nf_server_new_response(NULL, "application/x-pem-file", cert -> last_modified,
                            ...
                            ogs_error("%s", err);
                            ogs_assert(true ==
                                       nf_server_send_error(stream, 404, 3, message, "Certificate does not exists.",
                                                            err, NULL, m1_servercertificatesprovisioning_api,
                                                            app_meta));
                            ogs_free(err);
                            ...
                            ogs_error("%s", err);
                            ogs_assert(true ==
                                       nf_server_send_error(stream, 500, 3, message, "Certificate management problem.",
                                                            err, NULL, m1_servercertificatesprovisioning_api,
                                                            app_meta));
                            ogs_free(err);
                        }
                ...
                if (!strcmp(message -> h.resource.component[2], "content-hosting-configuration")) {
                    if (msaf_provisioning_session) {
                        ...
                        if (chc != NULL) {
                            ...
                            char * err = NULL;
                            err = ogs_msprintf(
                                    "Provisioning Session [%s]: Unable to retrieve the Content Hosting Configuration",
                                    message -> h.resource.component[1]);
                            ogs_error("%s", err);
                            ogs_assert(true == nf_server_send_error(stream, 404, 2, message,
                                                                    "Unable to retrieve the Content Hosting Configuration.",
                    ...
                    if (msaf_provisioning_session) {
                        ...
                        response = nf_server_new_response(NULL, "application/json",
                                                          CONTENT_PROTOCOLS_DISCOVERY_JSON_TIME,
                                                          CONTENT_PROTOCOLS_DISCOVERY_JSON_HASH,
                                                          msaf_self() -> config.server_response_cache_control -> m1_content_protocols_response_max_age,
                                                          NULL, m1_contentprotocolsdiscovery_api, app_meta);
                        ogs_assert(response);
                        nf_server_populate_response(response, strlen(CONTENT_PROTOCOLS_DISCOVERY_JSON),
                                                    msaf_strdup(CONTENT_PROTOCOLS_DISCOVERY_JSON), 200);
                        ogs_assert(true == ogs_sbi_server_send_response(stream, response));
                    } else {
                        char * err = NULL;
                        err = ogs_msprintf("Provisioning Session [%s] does not exist.",
                        ...
                        ogs_assert(true ==
                                   nf_server_send_error(stream, 404, 2, message, "Provisioning session does not exist.",
                                                        err, NULL, m1_contentprotocolsdiscovery_api, app_meta));
                        ogs_free(err);
                    }
                }
            } else if (message -> h.resource.component[1] && !message -> h.resource.component[2]) {
                ...
                if (provisioning_session && msaf_provisioning_session &&
                    !msaf_provisioning_session -> marked_for_deletion) {
                    ogs_sbi_response_t * response;
                    char * text;
                    text = cJSON_Print(provisioning_session);
                    ...
                    ogs_assert(true ==
                               nf_server_send_error(stream, 404, 2, message, "Provisioning session does not exists.",
                                                    err, NULL, m1_provisioningsession_api, app_meta));
                    ogs_free(err);
                }
                ...
                if (msaf_provisioning_session) {
                    ...
                    if (!strcmp(message -> h.resource.component[2], "content-hosting-configuration") &&
                        ...
                        if (!content_hosting_config) {
                            char * err = NULL;
                            err = ogs_msprintf(
                                    "While updating the Content Hosting Configuration for the Provisioning Session [%s], Failure parsing ContentHostingConfiguration JSON.",
                                    message -> h.resource.component[1]);
                            ogs_error("%s", err);
                            ogs_assert(true == nf_server_send_error(stream, 422, 2, message,
                                                                    "Bad ContentHosting Configuration JSON.", err, NULL,
                                                                    m1_contenthostingprovisioning_api, app_meta));
                            ogs_free(err);
                            break;
                        }
                        ...
                        if (rv) {
                            ...
                            char * err = NULL;
                            err = ogs_msprintf(
                                    "Provisioning Session [%s]: Update to Content Hosting Configuration failed.",
                                    message -> h.resource.component[1]);
                            ogs_error("%s", err);
                            ogs_assert(true == nf_server_send_error(stream, 404, 2, message,
                                                                    "Failed to update the contentHostingConfiguration.",
                                                                    err, NULL, m1_contenthostingprovisioning_api,
                                                                    app_meta));
                            ogs_free(err);
                        }
                        ...
                        if (msaf_provisioning_session) {
                            ...
                            if (!metrics_reporting_config) {
                                ...
                                cJSON * jsonScheme = cJSON_GetObjectItemCaseSensitive(metrics_reporting_config,
                                                                                      "scheme");
                                const char * scheme = NULL;
                                if (jsonScheme) scheme = cJSON_GetStringValue(jsonScheme);

                                cJSON * jsonDataNetworkName = cJSON_GetObjectItemCaseSensitive(metrics_reporting_config,
                                                                                               "dataNetworkName");
                                const char * dataNetworkName = NULL;
                                if (jsonDataNetworkName) dataNetworkName = cJSON_GetStringValue(jsonDataNetworkName);

                                cJSON * jsonIsReportingInterval = cJSON_GetObjectItemCaseSensitive(
                                        metrics_reporting_config, "isReportingInterval");
                                bool isReportingInterval = false;
                                if (jsonIsReportingInterval)
                                    isReportingInterval = cJSON_IsTrue(jsonIsReportingInterval);

                                cJSON * jsonReportingInterval = cJSON_GetObjectItemCaseSensitive(
                                        metrics_reporting_config, "reportingInterval");
                                int reportingInterval = 0;
                                if (jsonReportingInterval) reportingInterval = jsonReportingInterval -> valueint;

                                cJSON * jsonIsSamplePercentage = cJSON_GetObjectItemCaseSensitive(
                                        metrics_reporting_config, "isSamplePercentage");
                                bool isSamplePercentage = false;
                                if (jsonIsSamplePercentage) isSamplePercentage = cJSON_IsTrue(jsonIsSamplePercentage);

                                cJSON * jsonSamplePercentage = cJSON_GetObjectItemCaseSensitive(metrics_reporting_config,
                                                                                                "samplePercentage");
                                double samplePercentage = 0;
                                if (jsonSamplePercentage) samplePercentage = jsonSamplePercentage -> valuedouble;

                                cJSON * jsonUrlFilters = cJSON_GetObjectItemCaseSensitive(metrics_reporting_config,
                                                                                          "urlFilters");
                                OpenAPI_list_t * urlFilters = NULL;
                                if (jsonUrlFilters && cJSON_IsArray(jsonUrlFilters)) {
                                    int size = cJSON_GetArraySize(jsonUrlFilters);
                                ...
                                cJSON * jsonMetrics = cJSON_GetObjectItemCaseSensitive(metrics_reporting_config,
                                                                                       "metrics");
                                OpenAPI_list_t * metrics = NULL;
                                if (jsonMetrics && cJSON_IsArray(jsonMetrics)) {
                                    int size = cJSON_GetArraySize(jsonMetrics);
                                ...
                                msaf_metrics_reporting_configuration_t * updated_metrics_reporting_configuration = msaf_metrics_reporting_configuration_update(
                                        current_id,
                                        scheme,
                                        dataNetworkName,
                                        isReportingInterval,
                                ...
                                if (updated_metrics_reporting_configuration) {
                                    ...
                                    if (msaf_application_server_state_set_on_post(msaf_provisioning_session)) {
                                        ...
                                        char * text;
                                        cJSON * mrc_json = OpenAPI_metrics_reporting_configuration_convertToJSON(
                                                updated_metrics_reporting_configuration);
                                        if (mrc_json) {
                                            msaf_provisioning_session = msaf_provisioning_session_find_by_provisioningSessionId(
                                                    message -> h.resource.component[1]);
                                            response = nf_server_new_response(request -> h.uri, "application/json",
                                    ...
                                    char * err = NULL;
                                    err = ogs_msprintf(
                                            "Update of the Metrics Reporting Configuration failed for the Provisioning Session [%s]",
                                            message -> h.resource.component[1]);
                                    ogs_error("%s", err);
                                    ogs_assert(true == nf_server_send_error(stream, 500, 2, message,
                                                                            "Update of the Metrics Reporting Configuration failed.",
                                                                            err, NULL,
                                                                            m1_metricsreportingprovisioning_api,
                                                                            app_meta));
                                    ogs_free(err);
                            ...
                            ogs_assert(true == nf_server_send_error(stream, 404, 2, message,
                                                                    ...
                                                                    m1_metricsreportingprovisioning_api, app_meta));
                            ogs_free(err);
                        }
                            ...
                            for (hi = ogs_hash_first(request -> http.headers); hi; hi = ogs_hash_next(hi)) {
                                if (!ogs_strcasecmp(ogs_hash_this_key(hi), OGS_SBI_CONTENT_TYPE)) {
                                    if (ogs_strcasecmp(ogs_hash_this_val(hi), "application/x-pem-file")) {
                                        ...
                                        err = ogs_msprintf(
                                                "Unsupported Media Type: received type: %s, should have been application/x-pem-file",
                                                type);
                                        ogs_error("%s", err);

                                        ogs_assert(true == nf_server_send_error(stream, 415, 3, message,
                                                                                "Unsupported Media Type.", err, NULL,
                                                                                m1_servercertificatesprovisioning_api,
                                                                                app_meta));
                                        ogs_free(err);
                                        ogs_sbi_message_free(message);
                        ...
                        if (msaf_provisioning_session) {
                            ...
                            if (rv == 0 && provisioning_session_cert) {
                                ...
                                ogs_error("%s", err);
                                ogs_assert(true == nf_server_send_error(stream, 403, 3, message,
                                                                        "A server certificate already exist.", err,
                                                                        NULL, m1_servercertificatesprovisioning_api,
                                                                        app_meta));
                                ogs_free(err);
                                ...
                                ogs_error("%s", err);
                                ogs_assert(true == nf_server_send_error(stream, 404, 3, message,
                                                                        "Server certificate does not exist.", err, NULL,
                                                                        m1_servercertificatesprovisioning_api,
                                                                        app_meta));
                                ogs_free(err);
                            } else if (rv == 5) {
                                ...
                                ogs_assert(true == nf_server_send_error(stream, 400, 3, message,
                                                                        "CSR was never generated for the certificate.",
                                                                        err, NULL,
                                                                        m1_servercertificatesprovisioning_api,
                                                                        app_meta));
                                ogs_free(err);
                                ...
                                char * err = NULL;
                                err = ogs_msprintf("The public certificate [%s] provided does not match the key",
                                                   cert_id);
                                ogs_error("%s", err);
                                ogs_assert(true == nf_server_send_error(stream, 400, 3, message,
                                                                        "The public certificate provided does not match the key.",
                                                                        err, NULL,
                                                                        m1_servercertificatesprovisioning_api,
                                                                        app_meta));
                                ogs_free(err);
                                ...
                                char * err = NULL;
                                err = ogs_msprintf(
                                        "There was a certificate management problem for the certificate id [%s].",
                                        cert_id);
                                ogs_error("%s", err);

                                ogs_assert(true == nf_server_send_error(stream, 500, 3, message,
                                                                        "There was a certificate management problem.",
                                                                        err, NULL,
                                                                        m1_servercertificatesprovisioning_api,
                                                                        app_meta));
                                ogs_free(err);
                        ...
                        ogs_error("%s", err);
                        ogs_assert(true ==
                                   nf_server_send_error(stream, 404, 1, message, "Resource not found.", err, NULL,
                                                        m1_provisioningsession_api, app_meta));
                        ogs_free(err);
                    }
                } else {
                    ...
                    ogs_assert(true ==
                               nf_server_send_error(stream, 404, 3, message, "Provisioning session does not exist.",
                                                    err, NULL, m1_servercertificatesprovisioning_api, app_meta));
                    ogs_free(err);
                }
            ...
            if (message -> h.resource.component[1] && message -> h.resource.component[2] &&
                !strcmp(message -> h.resource.component[2], "certificates") &&
                message -> h.resource.component[3] &&
                !message -> h.resource.component[4]) {
                ...
                provisioning_session = msaf_provisioning_session_find_by_provisioningSessionId(
                        message -> h.resource.component[1]);
                ...
                if (provisioning_session) {
                    ...
                    if ((rv == 0) || (rv == 8)) {
                        ...
                        ogs_assert(true == ogs_sbi_server_send_response(stream, response));
                        msaf_provisioning_session_certificate_hash_remove(message -> h.resource.component[1],
                                                                          message -> h.resource.component[3]);
                        ...
                        char * err = NULL;
                        err = ogs_msprintf("Certificate management problem for certificate [%s].",
                                           message -> h.resource.component[3]);
                        ogs_error("%s", err);
            ...
            if (message -> h.resource.component[1] && message -> h.resource.component[2] &&
                ...
                if (!strcmp(message -> h.resource.component[2], "metrics-reporting-configurations")) {
                    ...
                    if (msaf_provisioning_session) {
                        int delete_result = msaf_metrics_reporting_configuration_delete(
                                message -> h.resource.component[3]);
                        if (delete_result == 0) {
                            ogs_sbi_response_t * response;
                            response = nf_server_new_response(NULL,
                ...
                if (!strcmp(message -> h.resource.component[2], "content-hosting-configuration")) {
                    ...
                    if (msaf_provisioning_session) {
                        if (msaf_provisioning_session && msaf_provisioning_session -> contentHostingConfiguration) {
                            ...
                            char * err = NULL;
                            err = ogs_msprintf("Provisioning Session [%s] has no Content Hosting Configuration.",
                                               message -> h.resource.component[1]);
                            ogs_error("%s", err);
                            ogs_assert(true == nf_server_send_error(stream, 404, 2, message,
                                                                    "Content Hosting Configuration does not exist.",
                                                                    err, NULL, m1_contenthostingprovisioning_api,
                                                                    app_meta));
                            ogs_free(err);
                        }
                ...
                if (!provisioning_session || provisioning_session -> marked_for_deletion) {
                    char * err = NULL;
                    err = ogs_msprintf("Provisioning session [%s] either not found or already marked for deletion.",
                                       message -> h.resource.component[1]);

                    ogs_error("%s", err);

                    ogs_assert(true == nf_server_send_error(stream, 500, 3, message,
                                                            "Provisioning session either not found or already marked for deletion.",
                                                            err, NULL, m1_provisioningsession_api, app_meta));
                    ogs_free(err);
                } else {
                    provisioning_session -> marked_for_deletion = 1;
                    response = nf_server_new_response(NULL, NULL, 0, NULL, 0, NULL, m1_provisioningsession_api,
                                                      app_meta);
                    ogs_assert(response);
            ...
            if (!strcmp(message -> h.resource.component[0], "provisioning-sessions")) {
                ...
                if (message -> h.resource.component[1]) {
                    ...
                    if (provisioning_session) {
                        if (message -> h.resource.component[2]) {
                            ...
                            if (!strcmp(message -> h.resource.component[2], "certificates")) {
                                if (message -> h.resource.component[3]) {
                                    ...
                                    cert = server_cert_retrieve(message -> h.resource.component[3]);
                                    if (cert) {
                                        methods = ogs_msprintf("%s, %s, %s, %s", OGS_SBI_HTTP_METHOD_GET,
                                                               OGS_SBI_HTTP_METHOD_PUT, OGS_SBI_HTTP_METHOD_DELETE,
                                ...
                                char * err = NULL;
                                err = ogs_msprintf("Method [%s]: Target [%s] not yet supported.", message -> h.method,
                                                   message -> h.resource.component[2]);
                                ogs_error("%s", err);
                                ogs_assert(true ==
                                           nf_server_send_error(stream, 404, 2, message, "Target not yet supported.",
                                                                err, NULL, NULL, app_meta));
                                ogs_free(err);
                            }
                        } else {
                            methods = ogs_msprintf("%s, %s, %s", OGS_SBI_HTTP_METHOD_GET, OGS_SBI_HTTP_METHOD_DELETE,
                                                   OGS_SBI_HTTP_METHOD_OPTIONS);
                            response = nf_server_new_response(request -> h.uri, NULL, 0, NULL, 0, methods,
                                                              m1_provisioningsession_api, app_meta);
                            nf_server_populate_response(response, 0, NULL, 204);
                        ...
                        err = ogs_msprintf("Method [%s]: [%s] - Provisioning Session [%s] does not exist.",
                                           message -> h.method, message -> h.resource.component[2],
                                           message -> h.resource.component[1]);
                        ogs_error("%s", err);
                        ogs_assert(true == nf_server_send_error(stream, 404, number_of_components, message,
                                                                "Provisioning Session does not exists.", err, NULL,
                                                                interface, app_meta));
                        ogs_free(err);
                    }
                ...
                ogs_error("%s", err);
                ogs_assert(true == nf_server_send_error(stream, 404, 0, message, "Target not yet supported.", err, NULL,
                                                        m1_provisioningsession_api, app_meta));
                ogs_free(err);
            }
            break;
            ...
            ogs_assert(true ==
                       nf_server_send_error(stream, OGS_SBI_HTTP_STATUS_FORBIDDEN, 0, message, "Invalid HTTP method.",
                                            message -> h.method, NULL, NULL, app_meta));
            END
            break;
            ...
            ogs_assert(true == nf_server_send_error(stream, OGS_SBI_HTTP_STATUS_BAD_REQUEST, 0, message,
                                                    "Invalid resource name", err, NULL, NULL, app_meta));
            ogs_free(err);
            END
            break;
            ...
            if (strcmp(message -> h.api.version, "v1") != 0) {
                ...
                ogs_error("%s", error);
                ogs_assert(true == nf_server_send_error(stream, OGS_SBI_HTTP_STATUS_BAD_REQUEST, 1, NULL,
                                                        "Not supported version", error, NULL, maf_management_api,
                                                        app_meta));
                ogs_free(error);
                break;
            }
            if (!message -> h.resource.component[0]) {
                ...
                ogs_error(error);
                ogs_assert(true == nf_server_send_error(stream, 404, 1, NULL, "Resource name required", error, NULL,
                                                        maf_management_api, app_meta));
                break;
            }
            ...
            if (provisioning_sessions) {
                response = nf_server_new_response(NULL, "application/json", 0, NULL,
                                                  msaf_self() -> config.server_response_cache_control -> m1_provisioning_session_response_max_age,
                                                  NULL, maf_management_api, app_meta);
                ...
                ogs_assert(true == nf_server_send_error(stream, OGS_SBI_HTTP_STATUS_INTERNAL_SERVER_ERROR, 0, message,
                                                        "Internal Server Error.", message -> h.method, NULL,
                                                        maf_management_api, app_meta));
            }
            DEFAULT
            ogs_error("Invalid HTTP method [%s]", message -> h.method);
            ogs_assert(true ==
                       nf_server_send_error(stream, OGS_SBI_HTTP_STATUS_FORBIDDEN, 0, message, "Invalid HTTP method.",
                                            message -> h.method, NULL, maf_management_api, app_meta));
            END
            break;
            ...
            ogs_assert(true ==
                       nf_server_send_error(stream, OGS_SBI_HTTP_STATUS_BAD_REQUEST, 1, message, "Invalid API name.",
        ...
        case OGS_EVENT_SBI_CLIENT:
            ...
            if (message -> h.resource.component[1] && message -> h.resource.component[2]) {
                ...
                if (!strcmp(message -> h.resource.component[2], "purge")) {
                    ...
                    if (response -> status == 204 || response -> status == 200) {
                            ...
                            for (hi = ogs_hash_first(request -> http.headers); hi; hi = ogs_hash_next(hi)) {
                                if (!ogs_strcasecmp(ogs_hash_this_key(hi), OGS_SBI_CONTENT_TYPE)) {
                                    if (ogs_strcasecmp(ogs_hash_this_val(hi), "application/json")) {
                                        ...
                                        ogs_assert(true == nf_server_send_error(stream, 415, 2, message,
                                                                                "Provisioning session does not exist.",
                                                                                err, NULL,
                                                                                m3_contenthostingprovisioning_api,
                                                                                app_meta));
                                        ogs_free(err);
                                        ogs_sbi_message_free(message);
                                        ogs_free(message);
                        ...
                        ogs_list_for_each_safe( & as_state -> purge_content_hosting_cache, next, content_hosting_cache) {
                            if (purge_node -> purge_regex) {
                                if (!strcmp(content_hosting_cache -> provisioning_session_id,
                                            purge_node -> provisioning_session_id) &&
                                    !strcmp(content_hosting_cache -> purge_regex, purge_node -> purge_regex))
                                    break;
                            } else if (!strcmp(content_hosting_cache -> provisioning_session_id,
                                               purge_node -> provisioning_session_id)) {
                                break;
                            }
                        }
                        if (content_hosting_cache) {
                            ...
                            ogs_debug("M1 List Purge refs: %d, Event Purge node refs: %d ",
                                      content_hosting_cache -> m1_purge_info -> refs, purge_node -> m1_purge_info -> refs);

                            purge_node -> m1_purge_info -> refs--;
                            ogs_debug(" After decrement, M1 List Purge refs: %d, Event Purge node refs: %d ",
                                      content_hosting_cache -> m1_purge_info -> refs, purge_node -> m1_purge_info -> refs);
                            if (!purge_node -> m1_purge_info -> refs) {
                                ...
                                ogs_sbi_response_t * response;
                                cJSON * purged_entries_total_json = cJSON_CreateNumber(
                                        purge_node -> m1_purge_info -> purged_entries_total);
                                ...
                                ogs_sbi_header_set(response -> http.headers, "Content-Type", "application/json");
                                ogs_assert(response);
                                ogs_assert(true == ogs_sbi_server_send_response(purge_node -> m1_purge_info -> m1_stream,
                                                                                response));

                                if (content_hosting_cache -> m1_purge_info)
                                    ogs_free(content_hosting_cache -> m1_purge_info);
                                if (content_hosting_cache -> provisioning_session_id)
                            ...
                            msaf_application_server_state_log( & as_state -> purge_content_hosting_cache,
                                                               "Purge Content Hosting Cache list");

                        }
                    }

                    if ((response -> status == 404) || (response -> status == 413) || (response -> status == 414) ||
                        (response -> status == 415) || (response -> status == 422) || (response -> status == 500) ||
                        (response -> status == 503)) {
                        char * error;
                        purge_resource_id_node_t * content_hosting_cache, * next = NULL;
                        ...
                        if (response -> status == 404) {
                            ...
                            ogs_error(
                                    "Error message from the Application Server [%s] with response code [%d]: Cache not found\n",
                                    as_state -> application_server -> canonicalHostname, response -> status);
                        } else if (response -> status == 413) {
                            ogs_error(
                                    "Error message from the Application Server [%s] with response code [%d]: Pay load too large\n",
                                    as_state -> application_server -> canonicalHostname, response -> status);
                        } else if (response -> status == 414) {
                            ogs_error(
                                    "Error message from the Application Server [%s] with response code [%d]: URI too long\n",
                                    as_state -> application_server -> canonicalHostname, response -> status);
                        } else if (response -> status == 415) {
                            ogs_error(
                                    "Error message from the Application Server [%s] with response code [%d]: Unsupported media type\n",
                                    as_state -> application_server -> canonicalHostname, response -> status);
                        } else if (response -> status == 422) {
                            ogs_error(
                                    "Error message from the Application Server [%s] with response code [%d]: Unprocessable Entity\n",
                                    as_state -> application_server -> canonicalHostname, response -> status);
                        } else if (response -> status == 500) {
                            ogs_error(
                                    "Error message from the Application Server [%s] with response code [%d]: Internal server error\n",
                                    as_state -> application_server -> canonicalHostname, response -> status);
                        } else if (response -> status == 503) {
                            ogs_error(
                                    "Error message from the Application Server [%s] with response code [%d]: Service Unavailable\n",
                                    as_state -> application_server -> canonicalHostname, response -> status);
                        } else {

                            ogs_error("Application Server [%s] sent unrecognised response code [%d]",
                                      as_state -> application_server -> canonicalHostname, response -> status);
                        }

                        if (purge_node -> purge_regex) {
                            error = ogs_msprintf("Application Server possibly encountered problem with regex %s",
                                                 purge_node -> purge_regex);
                        } else {
                        ...
                        ogs_assert(true == nf_server_send_error(purge_node -> m1_purge_info -> m1_stream,
                                                                response -> status, 3, &
                                                                        purge_node -> m1_purge_info -> m1_message,
                                                                "Problem occured during cache purge", error,
                                                                purge_cache_err, m1_contenthostingprovisioning_api,
                                                                app_meta));
                        ogs_free(error);
                        ...
                        ogs_list_for_each_safe( & as_state -> purge_content_hosting_cache, next, content_hosting_cache) {
                            if (purge_node -> purge_regex) {
                                if (!strcmp(content_hosting_cache -> provisioning_session_id,
                                    ...
                                    ogs_list_remove( & as_state -> purge_content_hosting_cache, content_hosting_cache);
                                    ogs_debug("M1 List Purge refs: %d, Event Purge node refs: %d ",
                                              content_hosting_cache -> m1_purge_info -> refs,
                                              purge_node -> m1_purge_info -> refs);
                                    if (content_hosting_cache -> m1_purge_info)
                                        ogs_free(content_hosting_cache -> m1_purge_info);
                                    if (content_hosting_cache -> provisioning_session_id)
                ...
                if (response -> status == 201) {
                    ...
                    ogs_debug("[%s] Method [%s] with Response [%d] recieved for Content Hosting Configuration [%s]",
                    ...
                    if (content_hosting_configuration) {
                        ...
                        ogs_debug("Removing %s from upload_content_hosting_configurations",
                                  content_hosting_configuration -> state);
                        ogs_list_remove( & as_state -> upload_content_hosting_configurations,
                                         content_hosting_configuration);
                        ogs_debug("Adding %s to current_content_hosting_configurations",
                                  content_hosting_configuration -> state);
                ...
                if (response -> status == 204) {
                    ...
                    if (as_state -> current_content_hosting_configurations) {
                        ...
                        ogs_list_for_each_safe(as_state -> current_content_hosting_configurations, next,
                                               content_hosting_configuration) {
                    ...
                    if (content_hosting_configuration) {
                        ...
                        msaf_application_server_state_log(as_state -> current_content_hosting_configurations,
                                                          "Current Content Hosting Configurations");

                        ogs_debug("Removing %s from current_content_hosting_configurations",
                                  content_hosting_configuration -> state);
                        ogs_free(content_hosting_configuration -> state);
                        ogs_list_remove(as_state -> current_content_hosting_configurations,
                                        content_hosting_configuration);
                        ogs_free(content_hosting_configuration);
                        msaf_application_server_state_log(as_state -> current_content_hosting_configurations,
                                                          "Current Content Hosting Configurations");
                    ...
                    ogs_list_for_each_safe( & as_state -> delete_content_hosting_configurations, node,
                                            delete_content_hosting_configuration) {

                        if (!strcmp(delete_content_hosting_configuration -> state, message -> h.resource.component[1])) {

                            msaf_application_server_state_log( & as_state -> delete_content_hosting_configurations,
                                                               "Delete Content Hosting Configurations");

                            ogs_debug("Destroying Content Hosting Configuration: %s",
                                      delete_content_hosting_configuration -> state);
                            ogs_free(delete_content_hosting_configuration -> state);
                            ogs_list_remove( & as_state -> delete_content_hosting_configurations,
                                             delete_content_hosting_configuration);
                ...
                ogs_assert(true == nf_server_send_error(stream, OGS_SBI_HTTP_STATUS_BAD_REQUEST, 0, message,
                                                        "Unknown M3 Content Hosting Configuration operation",
                                                        message -> h.resource.component[1], NULL, NULL, app_meta));
                break;
                END
                break;
            } else {
                ...
                if (response -> status == 200) {
                    ...
                    if (as_state -> current_content_hosting_configurations == NULL) {
                        as_state -> current_content_hosting_configurations = ogs_calloc(1,
                                                                                        sizeof( * as_state -> current_content_hosting_configurations));
                ...
                DEFAULT
                char * err = NULL;
                err = ogs_msprintf("Unknown M3 Content Hosting Configuration operation [%s] with method [%s]",
                                   message -> h.resource.component[1], message -> h.method);
                ogs_error("%s", err);
                ogs_assert(true == nf_server_send_error(stream, OGS_SBI_HTTP_STATUS_BAD_REQUEST, 0, message,
                                                        "Unknown M3 Content Hosting Configuration operation", err, NULL,
                                                        NULL, app_meta));
                ogs_free(err);
                break;
                END
            ...
            if (message -> h.resource.component[1]) {
                ...
                if (response -> status == 201) {
                    ...
                    ogs_debug("[%s] Method [%s] with Response [%d] recieved for certificate [%s]",
                              message -> h.resource.component[0], message -> h.method, response -> status,
                              message -> h.resource.component[1]);
                ...
                ogs_assert(true == nf_server_send_error(stream, OGS_SBI_HTTP_STATUS_BAD_REQUEST, 0, message,
                                                        "Unknown M3 certificate operation.",
                                                        message -> h.resource.component[1], NULL, NULL, app_meta));
                break;
                END
                break;
                ...
                DEFAULT
                char * err = NULL;
                err = ogs_msprintf("Unsupported M3 Certificate operation [%s] with method [%s]",
                                   message -> h.resource.component[1], message -> h.method);
                ogs_error("%s", err);
                ogs_assert(true == nf_server_send_error(stream, OGS_SBI_HTTP_STATUS_BAD_REQUEST, 0, message,
                                                        "Unknown M3 Certificate operation", err, NULL, NULL, app_meta));
                ogs_free(err);
                break;
                END
            ...
            ogs_assert(true == nf_server_send_error(stream, OGS_SBI_HTTP_STATUS_BAD_REQUEST, 0, message,
                                                    "Unsupported M3 operation", message -> h.resource.component[0], NULL,
                                                    NULL, app_meta));
            break;
            END
            break;
