...
void next_action_for_application_server(msaf_application_server_state_node_t *as_state) {
    ...
    ogs_free(component);
        ...
        const char *component =  ogs_msprintf("content-hosting-configurations/%s/purge", purge_chc->provisioning_session_id);
        ...
        m3_client_as_state_requests(as_state, purge_chc, "application/x-www-form-urlencoded", purge_chc->purge_regex, (char *)OGS_SBI_HTTP_METHOD_POST, component);
        } else {
        ogs_debug("M3 client: Sending Purge operation for cache [%s] to the Application Server", purge_chc->provisioning_session_id);
        m3_client_as_state_requests(as_state, purge_chc, "application/x-www-form-urlencoded", NULL, (char *)OGS_SBI_HTTP_METHOD_POST, component);
        }
        ogs_free(component); 
