                ...
                SWITCH(message->h.resource.component[0])
                    ...
                    CASE(OGS_SBI_HTTP_METHOD_DELETE)
                        if(message->h.resource.component[1] && !message->h.resource.component[2])
                        {
                            ...
                            dynamic_policy_event = (msaf_event_t*)populate_msaf_event_with_metadata(e, m5_dynamicpolicy_api,
                                                                                                    app_meta);
                        ...
                        if (message->h.resource.component[1] && !message->h.resource.component[2])
                        {
                            ...
                            if(!dynamic_policy) {
                                ...
                                ogs_assert(true == nf_server_send_error(stream, 404, 1, message,
                                                                        "Retrieving dynamic policy failed.", err, NULL,
                    ...
                    CASE(OGS_SBI_HTTP_METHOD_POST)
                        {
                            ...
                            if (!is_dynamic_policy_create_request_valid (request, stream, message, m5_dynamicpolicy_api, app_meta))
                                return;
                            ...
                            if(!msaf_dynamic_policy_create(dynamic_policy, dynamic_policy_event)) {
                                ...
                                ogs_assert(true == nf_server_send_error(stream, 400, 1, message,
                                                                        "Creation of the Dynamic Policy failed.", err, NULL,
                                                                        m5_dynamicpolicy_api, app_meta));
                                cJSON_Delete(dynamic_policy);
                                break;
                            }
                    ...
                    CASE(OGS_SBI_HTTP_METHOD_PUT)
                        if (message->h.resource.component[1] && !message->h.resource.component[2])
                        {
                            ...
                            if(!check_http_content_type(request->http,"application/json")){
                                ogs_assert(true == nf_server_send_error(stream, 415, 1, message, "Unsupported Media Type.",
                                                                        "Expected content type: application/json", NULL,
                                                                        m5_dynamicpolicy_api, app_meta));
                                ogs_sbi_message_free(message);
                                ogs_free(message);
                                return;
                            ...
                            if(!request->http.content) {
                                ogs_assert(true == nf_server_send_error(stream, 400, 1, message, "Bad request.",
                                                                        "Request has no content", NULL, m5_dynamicpolicy_api,
                                                                        app_meta));
                                ogs_sbi_message_free(message);
                                ogs_free(message);
                                return;
                            ...
                            if (!dynamic_policy) {
                                ...
                                ogs_assert(true == nf_server_send_error(stream, OGS_SBI_HTTP_STATUS_BAD_REQUEST, 1, message,
                                                                        "Malformed request body", err, NULL, m5_dynamicpolicy_api,
                                                                        app_meta));
                                ogs_free(err);
                                break;
                            }
                            ...
                            if(strcmp(message->h.resource.component[1], dynamic_policy->dynamic_policy_id)){
                                const char *err = "Updating dynamic policy: The path component and the JSON body have mismatching "
                                                  "Dynamic policy id";
                                ogs_error("%s", err);
                            ...
                            */
                            ...
                            if(!msaf_dynamic_policy_update_pcf(msaf_dynamic_policy, dynamic_policy)) {
                                ...
                                dyn_policy = msaf_api_dynamic_policy_convertResponseToJSON(msaf_dynamic_policy->DynamicPolicy);
                    ...
                    CASE(OGS_SBI_HTTP_METHOD_OPTIONS)
                        if (message->h.resource.component[1] && !message->h.resource.component[2]) {
                            ...
                            methods = ogs_msprintf("%s, %s, %s, %s",OGS_SBI_HTTP_METHOD_GET, OGS_SBI_HTTP_METHOD_PUT,
                                                   OGS_SBI_HTTP_METHOD_DELETE, OGS_SBI_HTTP_METHOD_OPTIONS);
                            response = nf_server_new_response(request->h.uri, NULL,  0, NULL, 0, methods, m5_dynamicpolicy_api,
                                                              app_meta);
                    ...
                    DEFAULT
                        ...
                        ogs_assert(true == nf_server_send_error(stream, OGS_SBI_HTTP_STATUS_FORBIDDEN, 1, message,
                                                                "Invalid HTTP method.", message->h.method, NULL, NULL, app_meta));
                    END
                    break;
                    ...
                    CASE(OGS_SBI_HTTP_METHOD_DELETE)
                        if(message->h.resource.component[1])
                        {
                            ...
                            if(na_sess){
                                ...
                                msaf_network_assistance_session_delete_by_session_id(message->h.resource.component[1]);
                            } else {
                                char *err = NULL;
                    ...
                    CASE(OGS_SBI_HTTP_METHOD_GET)
                        if(message->h.resource.component[1])
                        {
                            ...
                            network_assistance_sess = msaf_network_assistance_session_get_json(message->h.resource.component[1]);
                            if(network_assistance_sess) {
                                ...
                                na_sess = msaf_network_assistance_session_retrieve(message->h.resource.component[1]);
                                ...
                                char *err = NULL;
                                err = ogs_msprintf("The AF has no network assistance session with id [%s].",
                                                   message->h.resource.component[1]);
                                ogs_error("%s", err);

                                ogs_assert(true == nf_server_send_error(stream, 404, 0, message,
                                                                        "Unable to retrieve the Network Assistance Session", err,
                                                                        NULL, m5_networkassistance_api, app_meta));
                                ogs_free(err);
                            }
                    ...
                    CASE(OGS_SBI_HTTP_METHOD_POST)
                        if(message->h.resource.component[1] && !strcmp(message->h.resource.component[2], "boost-request"))
                        {
                            ...
                            if(!is_ue_allowed_to_request_delivery_boost(na_sess)) {
                                ...
                                reason = ogs_msprintf("The AF has an active delivery boost for the network assistance session "
                                                      "[%s].", message->h.resource.component[1]);
                                ogs_debug("%s", reason);

                                operation_success_response = msaf_api_operation_success_response_create(reason, 0);
                                op_success_response = msaf_api_operation_success_response_convertResponseToJSON(
                                                            operation_success_response);
                                success_response = cJSON_Print(op_success_response);

                                response = nf_server_new_response(NULL, "application/json", 0, NULL, 0, NULL,
                                                                  m5_networkassistance_api, app_meta);
                                ogs_assert(response);
                                nf_server_populate_response(response, strlen(success_response), ogs_strdup(success_response),
                                                            response_code);
                                ogs_assert(true == ogs_sbi_server_send_response(stream, response));
                            ...
                            nw_assist_event = (msaf_event_t*)populate_msaf_event_with_metadata(e, m5_networkassistance_api,
                                                                                               app_meta);
                            ...
                            if (!network_assistance_sess) {
                                ...
                                ogs_error("%s", err);
                                ogs_assert(true == nf_server_send_error(stream, 400, 0, message,
                                                                        "Creation of the Network Assistance Session failed.", err,
                                                                        NULL, m5_networkassistance_api, app_meta));
                                break;
                            }

                            service_data_flow_descriptions = cJSON_GetObjectItemCaseSensitive(network_assistance_sess,
                            ...
                            provisioning_session_id = cJSON_GetObjectItemCaseSensitive(network_assistance_sess,
                            ...
                            if(!msaf_nw_assistance_session_create(network_assistance_sess, nw_assist_event)) {
                                ...
                                const char *err = "Problem in obtaining the information required to create the Network Assitance "
                                                  "Session";
                                ogs_error("%s", err);
                                ogs_assert(true == nf_server_send_error(stream, 400, 1, message,
                                                                        "Creation of the Network Assistance Session failed.", err,
                ...
                CASE("service-access-information")
                    ...
                    CASE(OGS_SBI_HTTP_METHOD_GET)
                        ...
                        if (!sai_entry) {
                            ...
                            ogs_error("%s", err);
                            ogs_assert(true == nf_server_send_error(stream, 404, 1, message, "Provisioning Session not found.", err,
                                                                    NULL, m5_serviceaccessinformation_api, app_meta));
                            ogs_free(err);
                        } else {
                            ...
                            if (if_modified_since) {
                                ...
                                ogs_debug("IMS: sec=%i, min=%i, hour=%i, mday=%i, mon=%i, year=%i, gmtoff=%li", tm.tm_sec,
                                          tm.tm_min, tm.tm_hour, tm.tm_mday, tm.tm_mon, tm.tm_year, tm.tm_gmtoff);
                            ...
                            ogs_sbi_response_t *response;
                            response = nf_server_new_response(NULL, "application/json", ogs_time_sec(sai_entry->generated)+1,
                                                              sai_entry->hash,
                                                              msaf_self()->config.server_response_cache_control->
                                                                        m5_service_access_information_response_max_age,
                                                              NULL, m5_serviceaccessinformation_api, app_meta);
                            ogs_assert(response);
                            nf_server_populate_response(response, response_body?strlen(response_body):0, ogs_strdup(response_body),
                                                        response_code);
                            ogs_assert(true == ogs_sbi_server_send_response(stream, response));
                        }
                        break;
                    CASE(OGS_SBI_HTTP_METHOD_OPTIONS)
                        if (message->h.resource.component[1] && !message->h.resource.component[2]) {
                            ...
                            response = nf_server_new_response(request->h.uri, NULL,  0, NULL, 0,
                                                              OGS_SBI_HTTP_METHOD_GET ", " OGS_SBI_HTTP_METHOD_OPTIONS,
                                                              m5_serviceaccessinformation_api, app_meta);
                            ogs_assert(response);
                            nf_server_populate_response(response, 0, NULL, 204);
                            ogs_assert(true == ogs_sbi_server_send_response(stream, response));
                        } else {
                            ogs_assert(true == nf_server_send_error(stream, OGS_SBI_HTTP_STATUS_NOT_FOUND, 2, message, "Not Found",
                                                                    message->h.method, NULL, m5_serviceaccessinformation_api,
                                                                    app_meta));
                        }
                        break;
                    DEFAULT
                        ...
                        if (message->h.resource.component[1] && !message->h.resource.component[2]) {
                            ...
                            if (provisioning_session) {
                                if (!provisioning_session->consumptionReportingConfiguration) {
                                    ...
                                    err = ogs_msprintf("No ConsumptionReportingConfiguration for Provisioning Session [%s], cannot "
                                                       "accept reports", message->h.resource.component[1]);
                                    ogs_error("%s", err);
                                        ...
                                        if (json) {
                                            ...
                                            if (consumption_report) {
                                                if (msaf_data_collection_store(message->h.resource.component[1],
                                                    ...
                                                    err = ogs_msprintf("Failed to store Consumption Report for provisioning "
                                                                       "session [%s]", message->h.resource.component[1]);
                                                    ogs_error("%s", err);
                                                    ogs_assert(true == nf_server_send_error(stream,
                                                                                        OGS_SBI_HTTP_STATUS_INTERNAL_SERVER_ERROR,
                                                                                        1, message, "Data storage error", err, NULL,
                                                                                        m5_consumptionreporting_api, app_meta));
                                                    ogs_free(err);
                                                }
                                                ...
                                                err = ogs_msprintf("Badly formed ConsumptionReport posted for provisioning session "
                                                                   "[%s]: %s", message->h.resource.component[1], reason);
                                                ogs_error("%s", err);
                                                ogs_assert(true == nf_server_send_error(stream, OGS_SBI_HTTP_STATUS_BAD_REQUEST, 1,
                                                                                        message, "Malformed request body", err,
                                                                                        NULL, m5_consumptionreporting_api,
                                                                                        app_meta));
                                                ogs_free(err);
                                            }
                                            cJSON_Delete(json);
                                            ...
                                            err = ogs_msprintf("Badly formed request body when posting a consumption report for "
                                                               "provisioning session [%s]", message->h.resource.component[1]);
                                            ogs_error("%s", err);
                                            ogs_assert(true == nf_server_send_error(stream, OGS_SBI_HTTP_STATUS_BAD_REQUEST, 1,
                                                                                    message, "Malformed request body", err, NULL,
                                                                                    m5_consumptionreporting_api, app_meta));
                                            ogs_free(err);
                                        }
                                        break;
                                    DEFAULT
                                        char *err;
                                        err = ogs_msprintf("Unrecognised content type for Consumption Report for Provisioning "
                                                           "Session [%s]", message->h.resource.component[1]);
                                        ogs_error("%s", err);
                                        ogs_assert(true == nf_server_send_error(stream, OGS_SBI_HTTP_STATUS_UNSUPPORTED_MEDIA_TYPE,
                                                                                1, message, "Malformed request body", err, NULL,
                                                                                m5_consumptionreporting_api, app_meta));
                                        ogs_free(err);
                                    END
                                }
